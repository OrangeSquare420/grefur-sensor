...
#define MAX_ACTUATORS_ALLOWED 5
...
constexpr char FIRMWARE_VERSION[] = "0.3.2a";
...
class MqttManager {
  private:
    ...
    char apName[32];  // Privat medlemsvariabel

  public:
    MqttManager(PubSubClient& client) : mqttClient(client) {
      // Initialiser apName i konstruktøren
    ...
    // ========================
    // Core Publishing (uten apName parameter)
    // ========================
    void publishNestedValue(const char* subtopic, const char* payload) {
      char topic[128];
    ...
    void publishFlatValue(const char* subtopic, const char* value) {
      char topic[128];
      ...
      mqttClient.publish(topic, value, config.mqtt.retain);
    }

    // ========================
    // Data Publications (uten apName parameter)
    // ========================
    void publishMeasuredValue(float value, const char* sensorName, const char* unit) {
      if (config.mqtt.useNestedJson) {
        ...
        publishFlatValue(sensorName, valueStr);

        String topic = String(config.mqtt.baseTopic) + "/" + apName + "/" + sensorName + "/unit";
        mqttClient.publish(topic.c_str(), unit, config.mqtt.retain);
      }
    }
    ...
    void publishIP() {
      ...
      if (config.mqtt.useNestedJson) {
        ...
        publishNestedValue("ip", payload);
      } else {
        publishFlatValue("ip", ip.c_str());
      }
    }
    ...
    void publishStatus(const char* status) {
      if (config.mqtt.useNestedJson) {
        ...
        publishNestedValue("status", payload);
      } else {
        publishFlatValue("status", status);
        if (config.operation.publishRSSI) {
          char rssiStr[16];
          itoa(WiFi.RSSI(), rssiStr, 10);
          publishFlatValue("status/rssi", rssiStr);
        }
      }
    }
    ...
    void publishDeviceInfo() {
      if (config.mqtt.useNestedJson) {
        ...
        publishNestedValue("info", payload.c_str());
      } else {
        publishFlatValue("info/deviceId", String(ESP.getChipId(), HEX).c_str());
        publishFlatValue("info/deviceName", config.device.deviceName);
        publishFlatValue("info/location", config.device.location);
        publishFlatValue("info/ip", WiFi.localIP().toString().c_str());
      }
    }
    ...
    void publishWatchdog() {
      ...
      Serial.println("Gjeldende watchdog: " + String(watchdogCounter));

      char payload[128];
      snprintf(payload, sizeof(payload), "{\"value\":%d}", watchdogCounter);

      if (config.mqtt.useNestedJson) {
        publishNestedValue("watchdog", payload);
      } else {
        publishFlatValue("watchdog", String(watchdogCounter).c_str());
      }
    }
    ...
    // Hjelpefunksjon for å få tilgang til apName hvis nødvendig
...
void loop() {
  ...
  if (currentTime - lastPublishTime >= config.operation.publishInterval) {
    ...
    if (DEBUG) Serial.println("Runing sensor readings...");
    ...
    for (int i = 0; i < MAX_SENSORS_ALLOWED; i++) {
      ...
      if (!isnan(val)) {
        ...
        if (!publishOnlyChange || !isSame) {
          ...
          mqtt.publishMeasuredValue(val, sensorName.c_str(), unit.c_str());
